...
void fillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, vec3d &color)
{
    SDL_SetRenderDrawColor(gRenderer, color.x, color.y, color.z, 255);

	auto SWAP = [](int &x, int &y) { int t = x; x = y; y = t; };
	auto drawline = [&](int sx, int ex, int ny) { for (int i = sx; i <= ex; i++) SDL_RenderDrawPoint( gRenderer, i, ny); };
...
void fillTriangle(triangle &tri, vec3d &color){
	fillTriangle((int)tri.points[0].x, (int)tri.points[0].y, (int)tri.points[1].x, (int)tri.points[1].y, (int)tri.points[2].x, (int)tri.points[2].y, color);
}